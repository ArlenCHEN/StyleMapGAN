...
def sequential(*kargs):
    ...
    for layer in reversed(kargs):
        ...
        if hasattr(layer, 'out_features'):
            seq.out_features = layer.out_features
            break
...
def conv(in_channels, out_channels, kernel_size, stride=1, padding=0, init='kaiming', activation=nn.ReLU(), use_batchnorm=False):
    ...
    if use_batchnorm:
        convs.append(BatchNorm2d(out_channels))
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels=None, kernel_size=3, stride=1, padding=None, weight_init='kaiming', activation=nn.ReLU(), is_bottleneck=False, use_projection=False, scaling_factor=1.0):
